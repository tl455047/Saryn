...
u8 cmp_linear_search(afl_state_t *afl, u8* buf, u32 len, u32 cur, u64 cksum, u64 orig_v0, u64 orig_v1, FILE *f) {
...
u8 taint_fuzz(afl_state_t *afl, u8 *buf, u8 *orig_buf, u32 len, u8 mode) {  
  ...
  if (common_fuzz_cmplog_stuff(afl, orig_buf, len)) goto taint_fuzz_failed;
  ...
  for(; afl->stage_cur < afl->stage_max; afl->stage_cur++) {
    ...
    if (tmp[idx]->inst_type == CMP_TYPE_INS && 
      ...
      orig_v0 = afl->orig_cmp_map->log[tmp[idx]->id][tmp[idx]->hits].v0;
      orig_v1 = afl->orig_cmp_map->log[tmp[idx]->id][tmp[idx]->hits].v1;
 
      ret = cmp_linear_search(afl, buf, len, idx, cksum, orig_v0, orig_v1, f);
  ...
  // since this is expensive, for now we only apply this in direct mode
  if (afl->direct_mode) {
    ...
    for(; afl->stage_cur < afl->stage_max; afl->stage_cur++) {
      ...
      /*fprintf(f, "multi-cond: id: %06u hits: %06u type: %06u inst_type: %06u attr: %06u ret-addr: %08llx\n", 
          tmp[idx]->id, tmp[idx]->hits, tmp[idx]->type, tmp[idx]->inst_type, tmp[idx]->attr,
          tmp[idx]->ret_addr);*/
      ...
      if (tmp[idx]->inst_type == CMP_TYPE_INS && 
        ...
        afl->shm.cmp_map->headers[tmp[idx]->id].type = 0;
        if (common_fuzz_cmplog_stuff(afl, buf, len)) goto taint_fuzz_failed;

        orig_v0 = afl->shm.cmp_map->log[tmp[idx]->id][tmp[idx]->hits].v0;
        orig_v1 = afl->shm.cmp_map->log[tmp[idx]->id][tmp[idx]->hits].v1;
