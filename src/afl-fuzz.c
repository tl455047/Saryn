...
static void at_exit() {
  ...
  ptr = getenv(S2E_ENV_VAR);

  if (ptr && *ptr && (pid4 = atoi(ptr)) > 0) kill(pid4, SIGTERM);
  
  if (pid4 > 0) { kill(pid4, kill_signal); }
...
int main(int argc, char **argv_orig, char **envp) {
  ...
  while (likely(!afl->stop_soon)) {
    ...
    do {
      ...
      // if symbolic is terminated and is ready for synchronizing, we synchronize testcases
      // generated by symbolic engine. 
      if (afl->symbolic_mode && afl->ready_for_sync && !afl->stop_soon) {
        
        u8 *fn = alloc_printf("%s/testcase-", afl->s2e_out_dir);
        u8 *new_fn = alloc_printf("%s/queue", afl->s2e_out_dir);
        DIR *d;
